...
Prerequisite: run `python run.py train-vqvae` first to produce outputs/coco/checkpoints/vqvae.pt

Run: python run.py coco
"""
import sys
import os
...
from configs.coco import get_coco_config
...
def _build_coco_shared_enc(cfg, images, texts, img_tok, txt_tok, device):
    """Train Option B Shared Encoder on COCO with VQ-VAE image tokens + CLIP BPE text tokens."""
    # Shared encoder total vocab: BPE vocab + VQ-VAE codebook + 1
    ...
    class _TokenMLP(nn.Module):
        def __init__(self):
            ...
            self.pos = nn.Parameter(torch.randn(1, cfg.max_seq_len, cfg.emb_dim) * 0.02)
    ...
    print("  Tokenising COCO images and texts...")
    ...
    for ep in range(cfg.epochs):
        ...
        sched.step()
        if (ep + 1) % 5 == 0:
            print(f"  [COCO SharedEnc] Epoch {ep+1}/{cfg.epochs} loss={ep_loss/nb:.4f}")
    enc.eval()
    with torch.no_grad():
        ob_img = enc(img_toks).cpu().numpy()
        ob_txt = enc(txt_toks).cpu().numpy()
...
def run_coco():
    ...
    print("="*65)
    ob_img, ob_txt, shared_module, img_toks, txt_toks = _build_coco_shared_enc(
        cfg, images, texts, img_tok, txt_tok, device
    )

    # Dual encoder: same architecture, same tokenised data, no shared weights
    import torch.nn as nn
    import torch.nn.functional as F
    from training.losses import infonce
    import torch.optim as optim
    ...
    img_enc = _SingleMLP(img_vocab).to(device)
    txt_enc = _SingleMLP(txt_vocab).to(device)
    ...
    # Remap text tokens to [0, bpe_vocab) (already in this range)
    ...
    for ep in range(cfg.epochs):
        ...
        sched.step()
        if (ep + 1) % 5 == 0:
            print(f"  [COCO DualScratch] Epoch {ep+1}/{cfg.epochs} loss={ep_loss/nb:.4f}")
    img_enc.eval(); txt_enc.eval()
    with torch.no_grad():
        de_img = img_enc(img_toks_remap).cpu().numpy()
        de_txt = txt_enc(txt_toks_local).cpu().numpy()
    ...
    trainer.fit_tinyclip(tiny_clip, loader)
    tc_img = tiny_clip.encode_images(images)
    tc_txt = tiny_clip.encode_texts(texts)
    ...
    configs_eval = [
        ...
        {"name": "TinyCLIP (scratch COCO)","img_emb": tc_img, "txt_emb": tc_txt},
    ]
    ...
    dummy_cids = np.zeros(len(images), dtype=int)  # no concept labels in COCO
